...
function logToConsole() {
    if (log) console.log(new Date()
        .toISOString() + " [Controller]:", ...arguments);
}
...
Controller.prototype.updateGeoFabrikMetadata = function() {
    ...
    function walkSync(dir) {
        ...
        return fs.statSync(dir)
            .isDirectory() ? Array.prototype.concat(...fs.readdirSync(dir)
                .map(f => walkSync(path.join(dir, f)))) : dir;
    }
    ...
    for (var i in boundaryFileList) {
        ...
        var kml = new DOMParser()
            .parseFromString(fs.readFileSync(file, 'utf8'));
        var gj = geojsonFlatten(togeojson.kml(kml));
        gj.features.map(feature => {
            feature.properties.geofabrikName = file.substr(0, file.length - 4)
                .substr(5);
        ...
        this.workers.forEach(function(worker, idx, array) {
            if (worker.task.expirationDate) {
                ...
                if (expires < Date.now()) {
                    ...
                    var SQLdelete = this.api.db.prepare("DELETE FROM tasks WHERE id = ?;", worker.task.id);
                    SQLdelete.all(function(err, result) {
                        if (err) logToConsole("Error deleting task", worker.task.id, "from database", err);
                    });
...
function Worker(controller, task) {
    ...
    this.updateTask();
    this.updateIntervalID = setInterval(this.updateTask.bind(this), this.task.updateInterval * 1000);
}
Worker.prototype.findExtract = function(given, extractsGeoJSON) {
    ...
    var matches = extractsGeoJSON.features.filter(function(extract) {
        if (!turfInside(turfPoint(given.geometry.coordinates[0][0]), extract)) return false;
        ...
        if (erased === undefined) return true;
        return false;
    });
    if (matches.length === 0) {
        return undefined;
...
Worker.prototype.clipExtract = function(task, callback) {
    ...
    for (let i = 0; i < this.task.coverage.geometry.coordinates.length; i++) {
        var idx = i + 1;
        idx = (idx == 1 ? idx : -idx);
        var coords = this.task.coverage.geometry.coordinates[i].map(pair => pair.join("\t"));
        coords = coords.join("\n\t");
        polygons[i] = `${idx}\n\t${coords}\nEND`;
    }
    ...
    // clip extract
    const clippedpath = path.join(path.dirname(this.task.URL), "clipped_" + path.basename(this.task.URL));
    this.clipProcess = execFile('osmconvert', [this.task.URL, "-B=" + polypath, "-o=" + clippedpath], {
        maxBuffer: 1024 * 500
    }, function(error, stdout, stderr) {
        if (error) {
            logToConsole("[clipExtract] Error clipping data to coverage for task", this.task.id, "error:", error);
        ...
        if (mv.stderr.toString() !== '') {
            logToConsole(`[clipExtract] Error moving clipped file to original file.`, `mv stderr:\n ${mv.stderr}`);
            throw "Error moving clipped file.";
        }
        // remove poly-file
        const rm = spawnSync('rm', [polypath]);
        if (rm.stderr.toString() !== '') {
            logToConsole(`[clipExtract] Error removing poly-file for task ${this.task.id}`, `rm stderr:\n ${rm.stderr}`);
...
Worker.prototype.createInitialDatafile = function() {
    ...
    if (!this.controller.geofabrikMetadata) {
        logToConsole("Can't create initial data file for task", this.task.id, "- no Geofabrik metadata.");
        return;
    }
    var geofabrikName = this.findExtract(this.task.coverage, this.controller.geofabrikMetadata);
    if (geofabrikName === undefined) {
        logToConsole("Can't create initial data file for task", this.task.id, "- no Geofabrik extract found.");
        // ToDo Abort worker
        this.terminate();
        return;
    }
    ...
    var suffixIdx = geofabrikName.match(this.controller.geofabrikMetadir)
        .index;
    var suffix = geofabrikName.substr(suffixIdx + this.controller.geofabrikMetadir.length, geofabrikName.length) + "-latest.osm.pbf";
    logToConsole("[createInitialDatafile] Downloading", geofabrikBase + suffix, "for task", this.task.id);
    this.wgetInitialFile = execFile('wget', ['--progress=dot:giga', '-O', this.task.URL, geofabrikBase + suffix], {
        maxBuffer: 1024 * 1024
    }, function(error, stdout, stderr) {
        if (error) {
        ...
        if (stderr.match("saved")) {
            logToConsole("[createInitialDatafile] Successfully downloaded", geofabrikBase + suffix, "for task", this.task.id);
        }
...
Worker.prototype.updateTask = function() {
    ...
    if (this.controller.workers.filter(worker => worker.updateProcess !== undefined)
        .length >= this.controller.maxParallelUpdates) {
    ...
    if (!fs.existsSync(this.task.URL)) {
        logToConsole(`[updateTask] Can't update task ${this.task.id}.` + this.task.URL + " not found. Trying to initialize data file.");
        this.createInitialDatafile();
        return;
    }
    // check if file is older than threshold -> redownload
    var dateThreshold = 1;
    if ((Date.now() - fs.statSync(this.task.URL)
            .mtime) / 1000 / 60 / 60 / 24 > dateThreshold) {
        logToConsole(`${this.task.URL} older than ${dateThreshold} days, recreating.`);
        this.createInitialDatafile();
        return;
    }
    var newfile = path.join(path.dirname(this.task.URL), "new_" + path.basename(this.task.URL));
    // helper function that finishes update by inserting timings
    let finishTaskUpdate = function() {
        // insert timing into database
        var insertTimingSQL = this.controller.api.db.prepare(`INSERT INTO taskstats (timestamp, taskID, timing) 
                VALUES (?, ?, ?);`, new Date()
            .toISOString(), this.task.id, Date.now() - timing);
        insertTimingSQL.run(function(err) {
            if (err) logToConsole("[updateTask] SQL error:", err);
        });
        // update timing statistics
        var updateTimingStatsSQL = this.controller.api.db.prepare(`UPDATE tasks 
        ...
        // update lastUpdated
        var updateLastUpdatedSQL = this.controller.api.db.prepare(`UPDATE tasks 
             SET lastUpdated = ?
             WHERE id = ?`, new Date()
            .toISOString(), this.task.id);
    ...
    // start update
    this.updateProcess = execFile('osmupdate', ["-v", "--max-merge=2", `-t=osmupdate_temp/Task${this.task.id}`,
        ...
        if (error) {
            if (error.toString()
                .match("Your OSM file is already up-to-date.")) {
            ...
            if (stderr.match("Completed successfully")) {
                ...
                if (mv.stderr.toString() !== '') {
                    logToConsole(`[updateTask] Error moving new file to old file.`, `mv stderr:\n ${mv.stderr}`);
                } else {
...
Worker.prototype.terminate = function() {
    ...
    if (rm.stderr.toString() !== '') {
        logToConsole(`[terminateWorker] Error removing data for task ${this.task.id}`, `rm stderr:\n ${rm.stderr}`);
