...
api.use(morgan(':remote-addr - :remote-user [:date[clf]] ":method ' + ':url :body HTTP/:http-version" :status :res[content-length] :response-time ms ' + '":referrer" ":user-agent" ', {
...
api.db.run("CREATE TABLE if not exists tasks (id INTEGER PRIMARY KEY AUTOINCREMENT, " + "name TEXT NOT NULL, " + "coverage BLOB NOT NULL, " + "URL TEXT, " + "expirationDate TEXT, " + "updateInterval INT DEFAULT 600, " + "lastUpdated TEXT, " + "addedDate TEXT, " + "averageRuntime TEXT, " + "unique(coverage));");
api.db.run("CREATE TABLE if not exists taskstats (timestamp TEXT PRIMARY KEY, " + "taskID INTEGER, timing INTEGER);");
//
/// serve website
//
api.use(express.static('../web/'));
api.use('/data', serveIndex('./data/', {
    icons: true,
    view: "details"
}));
...
api.get('/api/tasks', function(req, res) {
    ...
    SQLselect.all(function(err, tasks) {
        if (err) res.status(500)
            .send("Error retrieving tasks from the database.");
        res.json(tasks);
    });
});
...
api.delete('/api/tasks', function(req, res) {
    ...
    SQLdelete.run(function(err) {
        ...
        res.status(200)
            .send("Succesfully deleted task with ID=" + req.body.id);
    });
});
api.post('/api/tasks', function(req, res) {
    ...
    if (!name || name.match(/^[a-zA-Z0-9]+$/) === null) errorlist.push("name [a-zA-Z]");
    if (typeof(coverage) != "object") {
        try {
            ...
            if (coverage.coordinates.reduce((result, subpoly) => subpoly.length + result, 0) > 10000) {
    ...
    if (errorlist.length === 0) {
        ...
        if (hint.length > 0) {
            errorlist.push("coverage: invalid polygon", JSON.stringify(hint));
        }
        ...
        if (expirationDate && isNaN(Date.parse(expirationDate))) errorlist.push("expirationDate in ISO 8601, hours optional" + "[YYYY-MM-DD(THH:MM:SS+HH:MM)]");
    }
    if (errorlist.length > 0) {
        res.status(400)
            .send("Error adding task. Invalid parameters: " + errorlist.join(", "));
        return;
    } else {
        // insert new task into database, sorry for callback hell,
        // can't think of another way to serialize. db.serialize did not work.
        var SQLinsert = api.db.prepare(`INSERT INTO tasks (name, coverage, expirationDate, addedDate, updateInterval) 
            VALUES (?, ?, ?, ?, ?);`, name, JSON.stringify(coverage), expirationDate, new Date()
            .toISOString(), updateInterval);
        logToConsole("POST task; SQL for insertion:", SQLinsert, "\nParameters:", name, coverage, expirationDate, new Date()
            .toISOString(), updateInterval);
        SQLinsert.run(function getID(err) {
            if (err) {
                logToConsole("SQL error:", err);
                res.status(500)
                    .send("POST task; Error inserting task:" + err);
                return;
            }
            logToConsole("Getting id...");
            // get id
            var id;
            var SQLselect = api.db.prepare("SELECT * FROM tasks WHERE name == ? AND " + "coverage == ?", name, JSON.stringify(coverage));
            logToConsole("POST task; SQL for id select;", SQLselect, "\nParameters:", name, coverage);
            SQLselect.all(function updateURL(err, rows) {
                if (err) {
                    logToConsole("SQL error:", err);
                    res.status(500)
                        .send("POST task; Error retrieving id " + "from database after insertion. Can't generate URL.");
                    return;
                }
                ...
                var SQLupdate = api.db.prepare("UPDATE tasks SET URL = ? WHERE id = ?", url, id);
                logToConsole("POST task; SQL for updating URL:", SQLupdate, "\nParameters:", url, id);
                SQLupdate.run(function(err) {
                    if (err) {
                        logToConsole("SQL error:", err);
                        res.status(500)
                            .send("POST task; Error updating task url:" + err);
                        return;
                    }
